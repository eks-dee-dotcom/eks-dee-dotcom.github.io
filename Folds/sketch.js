...
let innerBound = 100
let outerBound = 110
const speed = 0.005 //lower speed drastically increase draw qualtiy, but even at high speed eventually they reach the same level
const padding = 30
const peaks = 3
...
function setup() {
  ...
  if(canvasAuto){
    ...
    cx = windowWidth/2
    cy = windowHeight/2
    outerBound = min(windowHeight/2, windowWidth/2)*0.75
...
class dot{
  ...
  update(){
    ...
    this.dist = (innerBound + sin(this.angle*10)*padding*((this.i*4)+1))*0.8
